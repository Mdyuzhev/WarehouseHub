import { test, expect } from '@playwright/test'
import { ReceiptPage } from '../../pages/receipt.page'
import { DashboardPage } from '../../pages/dashboard.page'
import { loginAs, clearLocalStorage } from '../../utils/test-helpers'

test.describe('WH Receipts', () => {
  let receiptPage: ReceiptPage
  let dashboardPage: DashboardPage

  test.beforeEach(async ({ page }) => {
    await clearLocalStorage(page)
    receiptPage = new ReceiptPage(page)
    dashboardPage = new DashboardPage(page)

    await loginAs(page, 'admin')
    await page.waitForTimeout(500)
    await dashboardPage.selectFacility('WH-C-001')
    await expect(page).toHaveURL(/\/wh/, { timeout: 5000 })
  })

  test('WHR1: Incoming receipts list loads (from DC)', async ({ page }) => {
    await receiptPage.gotoList('wh')
    await page.waitForLoadState('networkidle')

    await expect(receiptPage.receiptsTable).toBeVisible({ timeout: 10000 })
  })

  test('WHR2: Receipt detail shows source facility (DC)', async ({ page }) => {
    await receiptPage.gotoList('wh')
    await page.waitForLoadState('networkidle')

    const hasRows = await page.locator('table tbody tr').count()

    if (hasRows > 0) {
      await receiptPage.clickRow(0)
      await page.waitForURL(/\/wh\/receipts\/\d+/, { timeout: 5000 })

      // Source facility should be displayed (DC-C-001)
      const sourceFacility = page.locator('[data-testid="source-facility"]')
      await expect(sourceFacility).toBeVisible()

      const sourceText = await sourceFacility.textContent()
      expect(sourceText).toContain('DC-C-001')
    } else {
      test.skip()
    }
  })

  test('WHR3: Confirm Receipt updates stock', async ({ page }) => {
    await receiptPage.gotoList('wh')
    await page.waitForLoadState('networkidle')

    const hasRows = await page.locator('table tbody tr').count()

    if (hasRows > 0) {
      // Find an APPROVED receipt to confirm
      await receiptPage.clickRow(0)
      await page.waitForURL(/\/wh\/receipts\/\d+/, { timeout: 5000 })

      const status = await receiptPage.getStatus()

      if (status.includes('APPROVED')) {
        // Confirm
        await receiptPage.confirm()
        await page.waitForTimeout(1000)

        // Status should change to CONFIRMED
        await expect(receiptPage.statusBadge).toContainText('CONFIRMED', { timeout: 5000 })

        // Stock should be updated (verify via stock page or API)
      }
    } else {
      test.skip()
    }
  })

  test('WHR4: COMPLETED Receipt hides action buttons', async ({ page }) => {
    await receiptPage.gotoList('wh')
    await page.waitForLoadState('networkidle')

    const hasRows = await page.locator('table tbody tr').count()

    if (hasRows > 0) {
      // Look for a COMPLETED receipt
      await receiptPage.clickRow(0)
      await page.waitForURL(/\/wh\/receipts\/\d+/, { timeout: 5000 })

      const status = await receiptPage.getStatus()

      if (status.includes('COMPLETED')) {
        // Action buttons should be hidden
        const approveVisible = await receiptPage.isApproveButtonVisible()
        expect(approveVisible).toBe(false)

        const confirmVisible = await receiptPage.confirmButton.isVisible({ timeout: 1000 })
          .catch(() => false)
        expect(confirmVisible).toBe(false)
      }
    } else {
      test.skip()
    }
  })
})
